...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!url) {
      ...
      if (all.length === 0) {
        return message.reply(
          `<a:AC_deny:902903694897590322> *You don't have any projects Added.*\nAdd one by using: ${default_prefix}add [project Url]`
        );
      }

      let embed = new MessageEmbed()
        .setColor("RANDOM")
        .setTitle("<a:AC_red_dot:902903574042902568> Select what project you wanna remove.");
      ...
      const msg = await message.author.send(embed).catch((err) => {
        ...
        if (err.message === "<a:AC_deny:902903694897590322> Cannot send messages to this user")
          message.channel.send(
            `<a:AC_deny:902903694897590322> Error: \`Cannot send message to you. please turn on your Dms\`.`
          );
      });
      ...
      collector.on("collect", async (reaction) => {
        switch (reaction.emoji.name) {
          case "1️⃣": {
            ...
            let embed = new MessageEmbed()
              .setTitle("<a:AC_success:904701594942185492> Removed Succesfully!")
    ...
    let waitEmbed = new MessageEmbed().setDescription(
      "<a:AC_red_dot:902903574042902568>  Please wait..."
    );
    ...
    if (checkIfExsists === null) {
      ...
      let embed = new MessageEmbed()
        .setTitle("<a:AC_deny:902903694897590322> Project is not Registered!")
